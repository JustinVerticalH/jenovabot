...
class WebScrapers(commands.Cog, name="Web Scrapers"):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post("http://headyversion.com/search/", data={"title": song_name, "csrfmiddlewaretoken": token}) as response:
                ...
                if str(response.url) == "http://headyversion.com/search/":
                    ...
                    if table is None:
                        return await interaction.response.send_message("Could not find a song with that title.", ephemeral=True)
    ...
    async def anilist_search(interaction: discord.Interaction, search: str, anime: bool=False, manga: bool=False):
        ...
        await interaction.response.send_message(response_url)
